...
fn main() {
    ...
    for index in (0..count).rev() {
        ...
        messages.push(
            serde_json::from_value::<MessageRecord>(serde_json::json!({
        ...
        recipients.push(RecipientRecord {
            ...
            recipient_key: format!("recipient_{index}"),
            recipient_type: "to".into(),
            display_name: None,
            raw_address: Some("synthetic@example.test".into()),
            address_type: None,
            smtp_address: None,
            resolution_status: "ok".into(),
            ordinal: 0,
        });
        bodies.push(text_body_payload(&key, &text));
        attachments.push(attachment_payload(
            &key,
            0,
            AttachmentMetadata {
                filename_original: Some("payload.bin".into()),
                ..Default::default()
            },
            vec![1, 2, 3, 4],
        ));
    ...
    let bytes = match mode.as_str() {
        "email" => {
            for record in iter_email_content_records(
                &messages,
                &[],
                &recipients,
                &body_records,
        ...
        "text" => {
            for record in iter_attachment_text_records(
        ...
        "disk" => {
            ...
            let (bytes, emitted) =
                write_disk_attachments(root.path(), &attachment_records, &attachments).unwrap();
