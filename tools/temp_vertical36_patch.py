...
module = r'''use std::collections::{BTreeMap, BTreeSet};
...
pub fn materialize_embedded_message_payloads(
    ...
    for attachment in attachments.iter_mut() {
        ...
        let Some(message) = messages.iter().find(|message| message.message_key == child_key) else {
            continue;
        };
        let child_recipients = recipients
            ...
            .cloned()
            .collect::<Vec<_>>();
        let Some(body) = bodies.iter().find(|body| {
            body.record.message_key == child_key && body.record.body_type == "text"
        }) else {
            continue;
        };
...
'''
Path("src/eml.rs").write_text(module)

replace_once("src/lib.rs", "pub mod engine;\n", "pub mod eml;\npub mod engine;\n")
