...
| IN-02 | ANSI version 14/15 traversal | Partial | Admitted Linux Rust Stage-A v14 structural fixture plus Stage-B one-folder/one-message fixture with exact populated BBT/NBT entries, flat ANSI String8 property contexts, contents/recipient table heaps, independent byte validation, repeat-run equality, PSTD canonical records, and exact plain-text EML; attachments, HTML/RTF, typed items, producer breadth, and malformed semantic derivatives remain open. |
