...
| IN-02 | ANSI version 14/15 traversal | Partial | Admitted Linux Rust Stage-A v14 structural fixture plus Stage-B one-folder/one-message fixture and Stage-C one-message/by-value-attachment fixture with exact populated BBT/NBT entries, flat ANSI String8 property contexts, contents/recipient/attachment heaps, independent byte validation, repeat-run equality, PSTD canonical records, exact plain-text EML, and exact inline/external attachment evidence; reference/embedded/OLE methods, HTML/RTF, typed items, producer breadth, and malformed semantic derivatives remain open. |
...
| ATT-03 | By-value method 1 | Partial | Direct and arbitrary-format payloads, 4/8-byte data trees, zero length, size mismatch, and unnamed attachment fallback are covered; #579 additionally proves one deterministic ANSI v14 property-context direct-binary layout with exact canonical, inline MIME/base64, external-file/manifest, hash, and fail-closed truncated-input evidence. Broad producer/layout corpus remains. |
