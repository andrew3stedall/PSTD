...
| ANSI input baseline | Stage B message plus Stage C direct/indirect by-value attachment exact for three controlled shapes | Deterministic Linux Rust v14 fixtures cover one `Synthetic Mail` folder/message with a plain-text body and structured To row, plus direct and HNID/SLBLOCK-resolved ANSI/String8 property-context method-1 attachment variants with exact filename, MIME, payload hash, canonical bytes, inline MIME/base64, external raw-file/manifest, and independent validation. Broad ANSI producers, methods 2/3/4, embedded/OLE attachments, HTML/RTF, and typed items remain unproven. |
...
ANSI Stage A is complete as a structural baseline. Stage B proves one controlled message extraction shape and Stage C proves direct and indirect controlled by-value attachment shapes; all remain below representative Purview Unicode coverage. Additional ANSI work must use separate fixtures for methods 2/3/4, embedded/OLE attachments, HTML/RTF bodies, non-mail items, malformed derivatives, and producer-specific layouts.
