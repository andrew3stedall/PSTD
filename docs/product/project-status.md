...
| ANSI input baseline | Stage B message plus Stage C direct/indirect method-1, method-2, and method-3 attachment exact for five controlled shapes | Deterministic Linux Rust v14 fixtures cover one `Synthetic Mail` folder/message with a plain-text body and structured To row, plus direct and HNID/SLBLOCK-resolved ANSI/String8 property-context method-1 variants and method-2/method-3 reference variants with exact method metadata, filename, MIME, payload hash, canonical bytes, inline MIME/base64, external raw-file/manifest, and independent validation. Broad ANSI producers, method 4, external ID2/path references, embedded/OLE attachments, HTML/RTF, and typed items remain unproven. |
