...
| ANSI input baseline | Stage B message plus Stage C direct/indirect method-1 and method-2 attachment exact for four controlled shapes | Deterministic Linux Rust v14 fixtures cover one `Synthetic Mail` folder/message with a plain-text body and structured To row, plus direct and HNID/SLBLOCK-resolved ANSI/String8 property-context method-1 variants and a method-2 reference variant with exact method metadata, filename, MIME, payload hash, canonical bytes, inline MIME/base64, external raw-file/manifest, and independent validation. Broad ANSI producers, methods 3/4, external ID2/path references, embedded/OLE attachments, HTML/RTF, and typed items remain unproven. |
