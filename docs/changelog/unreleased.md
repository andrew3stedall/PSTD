...
### ATT-04 ANSI method-3 by-reference attachment fixture

- Added a deterministic ANSI v14 method-3 attachment variant using the same validated
  HNID `0x311`/SLBLOCK payload mapping, while preserving `PR_ATTACH_METHOD=3` in
  canonical metadata and generated MIME/external output metadata.
- Extended the independent validator and workflow matrix to exercise direct, indirect
  method-1, method-2, and method-3 variants with exact payload hashes, inline MIME/base64,
  external files/manifests, repeatability, overwrite refusal, and truncation gates.
