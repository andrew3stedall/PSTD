...
| 2026-09-01 | Attachment vertical / #583 | ANSI method-4 by-reference attachment fixture | Added a deterministic ANSI v14 method-4 variant preserving `PR_ATTACH_METHOD=4` while resolving HNID `0x311` through the root SLBLOCK to the exact payload BID; direct, indirect method-1, method-2, method-3, and method-4 variants pass independent structure, canonical hash, inline MIME/base64, external manifest/raw-file, repeat, overwrite, and truncation gates. This is one controlled reference mapping, not external ID2/path or broad producer evidence. | External ID2/path layouts, embedded/OLE producer layouts, HTML/RTF, typed-item, malformed-derivative, and broader corpus fixtures. |
| 2026-09-01 | Attachment vertical / #584 | Attachment-table reference payload recovery | Threaded the bounded reader/BBT/parser-limit context into attachment-table subnode decoding and reused the validated HNID/SLBLOCK/data-tree resolver; unique table-row references now preserve exact bytes, method, size, MIME, filename, hash, and provenance, while unresolved/ambiguous/truncated cases remain unavailable. | External ID2/path layouts, producer breadth, and broader attachment corpus differentials. |
| 2026-09-06 | Vertical 45 / #590 | Standalone synthetic body attachment delivery | Validated RTF and opaque encrypted/encrypted-HTML body payloads are now emitted by `pstd-eml` as exact synthetic attachment-like records in inline MIME/base64 and external raw-file/manifest modes. Empty payloads remain represented; invalid, unresolved, and unavailable sources remain explicit without fabricated bytes. | Representative Purview Unicode attachment corpus covering remaining producer, reference, OLE, CID, and embedded-message Partial rows. |
...
Verticals 35-45 and the ANSI by-value/reference attachment slices are complete through the
merged #590 adapter delivery; they must not be duplicated. ANSI Stage A remains a structural
baseline, Stage B adds one controlled email/EML shape, and Stage C now covers direct and
indirect method-1 plus method-2, method-3, and method-4 HNID/SLBLOCK attachment layouts.
The next ANSI slices require separate fixtures for external ID2/path references,
embedded/OLE attachments, HTML/RTF bodies, typed items, malformed derivatives, and
producer-specific layouts. In parallel, an approved OLE-bearing PST or Purview Unicode
export with measured attachment bytes remains the highest-value external corpus target.
