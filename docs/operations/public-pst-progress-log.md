...
Verticals 35-40 are complete and must not be duplicated. ANSI Stage A is a structural baseline only; it adds no email or EML coverage. The next slice is an approved OLE-bearing PST or Purview Unicode export with measured attachment bytes, followed by broader ID2/reference layouts and output-profile differential evidence.
