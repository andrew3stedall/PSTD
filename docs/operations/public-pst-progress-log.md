...
Verticals 35-44 and the ANSI by-value attachment slice are complete when #579's workflow is green and merged; they must not be duplicated. ANSI Stage A remains a structural baseline, Stage B adds one controlled email/EML shape, and Stage C adds one controlled property-context by-value attachment. The next ANSI slices require separate fixtures for reference/embedded/OLE attachments, HTML/RTF bodies, typed items, malformed derivatives, and producer-specific layouts. In parallel, an approved OLE-bearing PST or Purview Unicode export with measured attachment bytes remains the highest-value external corpus target.
