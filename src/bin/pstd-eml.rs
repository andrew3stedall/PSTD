...
fn recipients_by_message(records: &[RecipientRecord]) -> BTreeMap<String, Vec<RecipientRecord>> {
    ...
    for record in records {
        grouped.entry(record.message_key.clone()).or_default().push(record.clone());
    }
...
fn bodies_by_message(payloads: &[BodyPayload]) -> BTreeMap<String, MessageBodies> {
    ...
    for payload in payloads {
        let entry = bodies.entry(payload.record.message_key.clone()).or_default();
...
fn decompress_rtf(input: &[u8]) -> Option<Vec<u8>> {
    ...
    match magic {
        MELA_MAGIC => {
            if expected_crc != 0 || compressed_size != raw_size || payload.len().checked_add(12)? != raw_size {
                return None;
            }
            Some(payload.to_vec())
        }
...
fn validated_transport_date(message: &MessageRecord) -> Option<String> {
    ...
    let mut dates = headers.lines().filter_map(|line| {
        ...
        name.eq_ignore_ascii_case("Date").then(|| clean_header(value)).flatten()
    });
...
fn recipient_header(records: &[RecipientRecord], role: &str) -> Option<String> {
    let values = records
        ...
        .filter_map(|record| {
            let address = record.smtp_address.as_deref().or(record.raw_address.as_deref()).and_then(clean_header)?;
...
fn clean_header(value: &str) -> Option<String> {
    ...
    let cleaned = value.chars().filter(|character| !character.is_control() || *character == '\t').collect::<String>();
...
mod tests {
    ...
    fn message() -> MessageRecord {
        MessageRecord {
            run_id: "run".to_string(), pst_id: "pst".to_string(), folder_key: "folder".to_string(), message_key: "message".to_string(), message_node_id: None, folder_path: "/Inbox".to_string(), item_type: "message".to_string(), subject: Some("Fixture subject".to_string()), sender_name: Some("Fixture Sender".to_string()), sender_email: Some("sender@example.com".to_string()), sender_raw_address: None, sender_address_type: Some("SMTP".to_string()), sent_at: None, received_at: None, created_at: None, modified_at: None, transport_message_headers: Some("Date: 19 Aug 2015 11:07:26 +0000\r\n".to_string()), internet_message_id: Some("<fixture@example.com>".to_string()), in_reply_to_id: None, conversation_index: None, conversation_topic: None, normalized_subject: None, has_text_body: true, has_html_body: false, has_attachments: false, attachment_count: 0, metadata_status: "ok".to_string(), threading_status: "ok".to_string(), body_status: "ok".to_string(), attachment_status: "none".to_string(), extraction_status: "ok".to_string(),
        }
    }

    fn recipient(ordinal: u64, role: &str) -> RecipientRecord {
        RecipientRecord { message_key: "message".to_string(), recipient_key: format!("recipient-{ordinal}"), recipient_type: role.to_string(), display_name: Some(format!("Recipient {ordinal}")), raw_address: Some(format!("r{ordinal}@example.com")), address_type: Some("native_email_address".to_string()), smtp_address: None, resolution_status: "validated".to_string(), ordinal }
    }

    #[test]
    fn emits_multipart_plain_and_rtf_message() {
        let bodies = MessageBodies { text: Some(b"Hello\nworld".to_vec()), rtf: Some(b"{\\rtf1\\ansi Rich body}".to_vec()) };
        let eml = build_eml(&message(), &[recipient(0, "to"), recipient(1, "cc")], &bodies).unwrap();
        let eml = String::from_utf8(eml).unwrap();
        assert!(eml.contains("Content-Type: multipart/alternative; boundary=\"pstd-alternative-7f6a8d2b\"\r\n"));
    ...
    fn fails_closed_for_missing_or_invalid_rtf_and_boundary_collision() {
        ...
        let missing = MessageBodies { text: Some(b"plain".to_vec()), rtf: None };
        assert!(build_eml(&message(), &recipients, &missing).is_none());
        let collision = MessageBodies { text: Some(ALTERNATIVE_BOUNDARY.as_bytes().to_vec()), rtf: Some(b"{\\rtf1 rich}".to_vec()) };
