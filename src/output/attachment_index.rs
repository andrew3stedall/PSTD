...
impl<'a> AttachmentIndex<'a> {
    pub fn new(records: &'a [AttachmentRecord], payloads: &'a [AttachmentPayload]) -> Self {
        ...
        let mut keys = records.iter().map(|record| record.attachment_key.as_str()).collect::<BTreeSet<_>>();
        ...
        for payload in payloads {
            ...
            indexed_payloads.entry(key).and_modify(|value| *value = None).or_insert(Some(payload));
        }
        records.sort_by(|left, right| {
            (&left.message_key, left.ordinal, &left.attachment_key)
                .cmp(&(&right.message_key, right.ordinal, &right.attachment_key))
        });
        Self { records, payloads: indexed_payloads }
    }
