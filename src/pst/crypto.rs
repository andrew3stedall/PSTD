use crate::error::{PstdError, PstdResult};

pub const NDB_CRYPT_NONE: u8 = 0;
pub const NDB_CRYPT_PERMUTE: u8 = 1;
pub const NDB_CRYPT_STRONG: u8 = 2;

const PERMUTE_DECODE_TABLE: [u8; 256] = [
    ,
];
const COMP_HIGH1: [u8; 256] = [
    ,
];
const COMP_HIGH2: [u8; 256] = [
    ,
];

const fn inverse_permutation(table: [u8; 256]) -> [u8; 256] {
    let mut inverse = [0u8; 256];
    let mut index = 0usize;
    while index < 256 {
        inverse[table[index] as usize] = index as u8;
        index += 1;
    }
    inverse
}

const PERMUTE_ENCODE_TABLE: [u8; 256] = inverse_permutation(PERMUTE_DECODE_TABLE);

pub fn decode_in_place(i_id: u64, bytes: &mut [u8], method: u8) -> PstdResult<&'static str> {
    match method {
        NDB_CRYPT_NONE => Ok("payload_loaded_unencoded"),
        NDB_CRYPT_PERMUTE => {
            for byte in bytes {
                *byte = PERMUTE_DECODE_TABLE[*byte as usize];
            }
            Ok("payload_loaded_permute_decoded")
        }
        NDB_CRYPT_STRONG => {
            let mut salt =
                ((((i_id & 0x0000_0000_ffff_0000) >> 16) ^ (i_id & 0x0000_0000_0000_ffff)) as u16);
            for byte in bytes {
                let losalt = (salt & 0x00ff) as u8;
                let hisalt = (salt >> 8) as u8;
                let mut value = (*byte).wrapping_add(losalt);
                value = COMP_HIGH1[value as usize];
                value = value.wrapping_add(hisalt);
                value = COMP_HIGH2[value as usize];
                value = value.wrapping_sub(hisalt);
                value = PERMUTE_DECODE_TABLE[value as usize];
                *byte = value.wrapping_sub(losalt);
                salt = salt.wrapping_add(1);
            }
            Ok("payload_loaded_strong_crypt_decoded")
        }
        unsupported => Err(PstdError::pst_parse(
            None,
            format!("unsupported PST data block crypt method 0x{unsupported:02x}"),
        )),
    }
}

pub fn encode_permutative(bytes: &[u8]) -> Vec<u8> {
    bytes
        .iter()
        .map(|byte| PERMUTE_ENCODE_TABLE[*byte as usize])
        .collect()
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn permutative_table_matches_source_inverse() {
        for byte in 0..=u8::MAX {
            let encoded = encode_permutative(&[byte]);
            let mut decoded = encoded;
            decode_in_place(0, &mut decoded, NDB_CRYPT_PERMUTE).unwrap();
            assert_eq!(decoded, vec![byte]);
        }
    }

    #[test]
    fn strong_crypt_known_vector_matches_pinned_libpst() {
        let mut encrypted = vec![
            0x6f, 0xab, 0x36, 0xbf, 0xbe, 0x12, 0x8e, 0x2b, 0xa8, 0xc4, 0xa6, 0x33, 0xd9, 0x09,
            0x61, 0xbe, 0x75,
        ];
        decode_in_place(0x12345678, &mut encrypted, NDB_CRYPT_STRONG).unwrap();
        assert_eq!(encrypted, b"strong-crypt-test");
    }

    #[test]
    fn unknown_method_fails_closed() {
        let error = decode_in_place(0, &mut [0u8; 1], 7).unwrap_err();
        assert!(error
            .to_string()
            .contains("unsupported PST data block crypt method"));
    }
}
